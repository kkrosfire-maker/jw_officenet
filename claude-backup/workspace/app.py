...
CHUNK = 500  # 취소 버튼 응답성을 위해 몇 행씩 나눠서 계산

# 한글 단위 표기 → 영문 약어 정규화 패턴
_UNIT_PATTERNS = [
    (r'[미밀]리그[람램]',    'mg'),
    (r'[미밀]리리터',        'ml'),
    (r'마이크로그[람램]',    'mcg'),
...
def _group_key(s: str) -> tuple:
    """(첫 글자, 숫자 목록) — 단위 정규화 후 추출하여 숫자가 정확히 같아야 비교"""
    norm = _normalize(s)
...
class FuzzyMatchApp:
        ...
        try:
            ...
            for key, idx_a in groups_a.items():
                ...
                if key not in groups_b:
                    processed += len(idx_a)
                    self.root.after(0, self._update_progress, processed / n * 90)
                    continue
                ...
                for start in range(0, len(idx_a), CHUNK):
                    ...
                    for ii, i in enumerate(chunk_ia):
                        ...
                        for jj, j in enumerate(idx_b):
                            kb = kor_b[j]
                            # 한글이 정확히 일치하거나, 한쪽이 다른 쪽에 포함될 때만 유사도 반영
    ...
    def _set_computing(self, computing: bool):
        if computing:
            ...
            self.progress_frame.pack(fill="x", pady=(0, 4), before=self.progress_frame.master.winfo_children()[0]
                                      if False else None)
