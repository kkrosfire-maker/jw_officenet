...
| 파일 | 역할 |
|------|------|
| `output/[주제]/draft.md` | 이미지 경로가 치환된 최종 글 원본 |
| `output/[주제]/images/` | 실제 PNG 파일들 (경로 확인용) |
...
1. `output/[주제]/draft.md` 읽기
2. `output/[주제]/images/` 폴더의 파일 목록 확인
...
draft.md의 내용을 그대로 복사해 `output/[주제]/final.md`로 저장한다.
...
- HTML 파일이 `output/[주제]/`에 저장되므로 로컬 파일로 바로 열 수 있어야 함
